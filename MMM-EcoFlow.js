Module.register("MMM-EcoFlow", {
  ...
  start: function () {
    ...
    this.hidden =
      this.config && typeof this.config.showModule === "boolean"
  ...
  getDom: function () {
    ...
    const relativeTime = this.lastReceivedAt
      ? this.formatRelativeTime(this.lastReceivedAt)
      : this.lastUpdate;
    const totalLabel =
      this.entryCount > 0 ? ` (${this.entryCount} entries total)` : "";
    updateDiv.innerHTML =
      "<span class='dimmed'>Last Data:</span> " + relativeTime + totalLabel;
